...
module.exports = {
    ...
    getUnitFromArray: function (unitArray, replyData) {
        ...
        let unitCode = unitArray.filter((value) =>
            unitKeys.includes(value.substring(0, 2).toLowerCase()),
        )
        const isNaval = unitArray.filter(
            (value) =>
                value.includes('rf') ||
                value.includes('sc') ||
                value.includes('bo') ||
        ...
        const rangeOverride = unitArray.filter(
            (value) => value === 'r' || value === 'nr',
        )

        const currentHPArray = unitArray.filter(
            (x) => !isNaN(parseInt(x)) || x === 'v',
        )
        ...
        if (currentHPArray.length > 0) unit.setHP(currentHPArray, replyData)

        const defenseBonusArray = unitArray.filter(
            (value) =>
                value.toLowerCase() === 'w' || value.toLowerCase() === 'd',
        )
        if (defenseBonusArray.length > 0)
            unit.addBonus(defenseBonusArray, replyData)

        const navalUnitArray = unitArray.filter(
            (value) =>
                value.includes('rf') ||
                value.includes('sc') ||
                value.includes('bo') ||
        ...
        if (navalUnitArray.length > 0) {
            if (unit.onTheWater) unit.onTheWater(navalUnitArray)
            else
                throw `Are you really trying to put the **${unit.name}** in a naval unit...`
        }

        const final = unitArray.filter((value) => value.toLowerCase() === 'f')
        if (final.length > 0) unit.final = true
        else unit.final = false

        if (rangeOverride[0] === 'r') unit.retaliation = true
        if (rangeOverride[0] === 'nr') unit.retaliation = false

        const toPoison = unitArray.filter(
            (value) => value.toLowerCase() === 'p',
        )
        ...
        if (toPoison.length > 0 && toBoost.length > 0) {
            ...
            if (toPoison.length > 0) poison(unit)

            if (toBoost.length > 0 && unit.att !== 0) boost(unit)
        }
    ...
    // eslint-disable-next-line no-unused-vars
    getBothUnitsArray: function (args) {
